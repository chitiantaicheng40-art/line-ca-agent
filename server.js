...
「転職相談したい」「面接対策したい」「この求人どう？」「求人比較したい」など、自由に話しかけてください。
...
function defaultJobCompareMode() {
  return {
    active: false,
    last_compare_text: null,
  };
}
...
async function getSession(userId) {
  const { data, error } = await supabase
    ...
    .select("user_id, history, profile, summary, updated_at, interview_state, job_mode, job_compare_mode")
    .eq("user_id", userId)
    .maybeSingle();
  ...
  if (!data) {
    const init = {
      ...
      job_compare_mode: defaultJobCompareMode(),
  ...
  return {
    ...
    job_compare_mode: data.job_compare_mode || defaultJobCompareMode(),
...
async function saveSession(session) {
  const { error } = await supabase.from("line_ca_sessions").upsert({
    ...
    job_compare_mode: session.job_compare_mode || defaultJobCompareMode(),
    updated_at: new Date().toISOString(),
  });
...
function isJobReviewStartMessage(text) {
  return /この求人どう|求人を見て|求人見て|求人票を見て|この案件どう|このポジションどう/.test(text || "");
}
...
function isJobCompareStartMessage(text) {
  return /求人比較したい|求人を比較したい|この3求人比較|複数求人を比較|求人比較して/.test(text || "");
}

function isJobCompareEndMessage(text) {
  return /求人比較終了|比較モード終了|通常相談に戻る|通常モード|終了/.test(text || "");
}
...
function looksLikeJobComparisonText(text) {
  if (!text) return false;
  const hasMultiLabels = /求人A|求人B|求人C|A:|B:|C:|1\.|2\.|3\./.test(text);
  const hasJobWords = /仕事内容|必須条件|歓迎条件|年収|勤務地|ポジション|職務内容/.test(text);
  return hasMultiLabels || hasJobWords;
}
...
- 6文目で次に確認すべきことを1つだけ質問
  ...
  return (
    response.output_text ||
    "現時点では条件付きでありです。職種との接続は見えますが、再現性と期待役割にまだ確認したい点があります。今の営業スタイルをもう少し詳しく教えてください。"
  ).trim();
}

// ===== 複数求人比較 =====
function buildJobCompareStartMessage() {
  return `了解です。比較したい求人をまとめて貼ってください。
...
貼ってもらえれば、1位・2位・3位とその理由まで返します。`;
}

async function compareJobs(session, compareText) {
  const profile = session.profile || defaultProfile();
...
不安点: ${profile.concerns || "未取得"}
  ...
  const prompt = `
あなたは世界一のリクルーティングアドバイザーです。
候補者プロフィールと複数求人を比較し、どの求人を優先して受けるべきか順位づけしてください。
...
- 甘い評価は禁止
  ...
  return (
    response.output_text ||
    "現時点では求人Bが最有力です。今の経験との再現性が最も高く、次に求人A、最後に求人Cの順です。まずはBから受ける前提で進めたいですが、新規開拓より既存深耕の方が得意ですか？"
  ).trim();
}
...
app.post("/webhook", async (req, res) => {
  ...
  for (const event of events) {
    try {
      ...
      // ===== 比較モード中に相談系が来たら自動解除 =====
      if (session.job_compare_mode?.active && shouldSwitchToNormalMode(userText) && !looksLikeJobComparisonText(userText)) {
        session.job_compare_mode = defaultJobCompareMode();
      }
      ...
      // ===== 比較モード終了 =====
      if (isJobCompareEndMessage(userText) && session.job_compare_mode?.active) {
        session.job_compare_mode = defaultJobCompareMode();
        session.history.push({ role: "user", content: userText });

        const endReply =
          "了解です。求人比較モードを終了して、通常のキャリア相談モードに戻します。比較したい時は『求人比較したい』と送ってください。";
      ...
      // ===== 比較モード開始 =====
      if (isJobCompareStartMessage(userText)) {
        session.job_compare_mode = {
          active: true,
          last_compare_text: null,
        };
        ...
        const startReply = buildJobCompareStartMessage();
        session.history.push({ role: "assistant", content: startReply });
      ...
      // ===== 比較モード中 =====
      if (session.job_compare_mode?.active) {
        session.profile = updateProfileFromUserMessage(session.profile, userText);
        session.history.push({ role: "user", content: userText });

        if (!looksLikeJobComparisonText(userText)) {
          const askReply =
            "比較したい求人をまとめて貼ってください。求人A・求人B・求人Cのように分けてもらえると、かなり精度高く順位づけできます。";
        ...
        session.job_compare_mode.last_compare_text = userText;

        const result = await compareJobs(session, userText);
